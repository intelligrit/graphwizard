...
func TestNode2VecWalksParallel_Triangle(t *testing.T) {
	...
	walks := Node2VecWalksParallel(g, WalkParams{
		WalkLength:   5,
		WalksPerNode: 2,
		P:            1.0,
		Q:            1.0,
...
func TestNode2VecWalksParallel_Empty(t *testing.T) {
	g := simple.NewUndirectedGraph()
	walks := Node2VecWalksParallel(g, WalkParams{WalkLength: 5, WalksPerNode: 1, P: 1, Q: 1}, 42)
	if walks != nil {
...
func TestDeepWalkWalksParallel_Triangle(t *testing.T) {
	...
	walks := DeepWalkWalksParallel(g, 5, 2, 42)
	if len(walks) != 6 {
		t.Fatalf("expected 6 walks, got %d", len(walks))
	}
}
