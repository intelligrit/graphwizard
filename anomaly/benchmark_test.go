...
// attachment. Inlined here to avoid circular imports with benchmark/.
...
func BenchmarkIsolationScore_1K(b *testing.B) {
...
func BenchmarkDegreeZScore_10K(b *testing.B) {
	rng := rand.New(rand.NewSource(42))
...
func BenchmarkStructuralOutliers_1K(b *testing.B) {
	rng := rand.New(rand.NewSource(42))
	g := barabasiAlbert(1000, 3, rng)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		StructuralOutliers(g, 10)
	}
}
